...
return (<>
<motion.div className='flex justify-center gap-10 items-start p-5  border-[#ffffff4d] bg-gradient-to-t from-[#ffad161f] to-[rgb(8, 8, 8)] mt-20 will-change-transform will-change-opacity' variants = {slideChildContainer}>
...
<fieldset className='flex flex-col items-center justify-center text-white border-1 border-[#807e7e65] rounded-2xl' >
<motion.div className='flex items-start flex-col gap-2 p-5 text-white mt-2 lg:flex-col  ' variants = {slideChildContainer} >
...
<input type = 'text' className='bg-[#80808023] rounded-2xl md:min-w-160 sm:min-w-100 min-h-11 p-3 placeholder:text-sm'   placeholder='Your Full Name...' value = {field.name}
...
<input type = 'email' className='bg-[#80808023]  rounded-2xl min-w-160 min-h-11 p-3 placeholder:text-sm' placeholder='Your Email...'value = {field.email} onChange={(e) => handleField('email', e.target.value.trim())} 
...
<textarea className='bg-[#80808023] rounded-2xl min-w-160 p-3 min-h-50'  placeholder='Leave a message...' value = {field.message} ref ={inputRef[2]} onChange={(e) => handleField('message',e.target.value)}  name="message"></textarea>
<button type = 'submit' 
className=
